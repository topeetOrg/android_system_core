...
void device_init(void)
{
    ...
    if (stat(coldboot_done, &info) < 0) {
        ...
        coldboot("/sys/devices");
                ...
                //~ MT6620 nodes, should remove if MTK update driver
                //~ behavior for current Android
        make_device("/dev/stpwmt", NULL, 0, 190, 0);
        make_device("/dev/stpgps", NULL, 0, 191, 0);
        //make_device("/dev/stpbt", NULL, 0, 192, 0);
        make_device("/dev/fm", NULL, 0, 193, 0);
        make_device("/dev/wmtWifi", NULL, 0, 194, 0);
